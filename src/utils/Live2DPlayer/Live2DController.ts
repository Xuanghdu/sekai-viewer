...
export class Live2DController extends Live2DPlayer {
  ...
  // Instead of a single lastAudioStep, we now store a history of steps.
  public actionHistory: number[] = [];
   ...
   * This saves the current scenario step (and any other state if needed)
   * to allow restoring the canvas later.
   */
  pushHistory(step: number): void {
    this.actionHistory.push(step);
    console.log("pushHistory", this.actionHistory);
  }
  ...
  create_model_queue = (queue_max = 6) => {
    ...
    // collect costumes needed in each action
      ...
      switch (action.Action) {
        ...
        case SnippetAction.Talk: {
          ...
          costume_list.push(
            ...detail.Motions.map(
              (mo) =>
                current_costume.find((m) => m.cid === mo.Character2dId)!.costume
            )
          );
          break;
        }
  ...
  step_until_checkpoint = async (step: number) => {
    ...
    // continue if the next steps have ProgressBehavior = Now
    while (
      !is_end(current) &&
    ...
    // wait for all talk sounds to finish
  ...
  /**
   * Apply motions. If the model is in T-pose and the motion or expression is empty,
   * apply the last motion/expression.
   */
  ...
  set_volume = (volume: {
    ...
    if (volume.bgm_volume) this.settings.bgm_volume *= 0.5;
